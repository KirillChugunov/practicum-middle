...
export default class ChatCard extends Block {
    ...
    <div class="chatCard__info">
        ...
        <p>{{lastMessagePreview.owner}}</p><small>{{lastMessagePreview.message}}</small>
        </div>
    </div>
    <div class="chatCard__notifications">
        <small>{{lastMessagePreview.timeStamp}}</small>
        <div class="chatCard__notification-container"><small class="chatCard__notification-text">{{newMessageCount}}</small></div>
    </div>
    `;
    }
}
