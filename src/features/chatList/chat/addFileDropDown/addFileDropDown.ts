...
export default class AddFileDropDown extends Block {
    ...
    public render(): string {
        return `
    <div class="add-file__option">{{{ PhotoButton  }}}<p>Фото или
        Видео</p></div>
    <div class="add-file__option">{{{ FileButton  }}}<p>Файл</p></div>
    <div class="add-file__option">{{{ LocationButton }}}<p>Локация</p></div>
    `;
    }
}
