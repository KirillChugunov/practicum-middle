...
class UserStore extends Store<TUser> {
  ...
  public async updateProfile(profileData: ProfileUpdatePayload): Promise<void> {
    try {
      ...
      if (res.status === 200) {
        ...
        console.error('Ошибка при обновлении профиля:', res.status, res.responseText);
      }
    } catch (error) {
  ...
  public async updatePassword(profileData: PasswordUpdatePayload): Promise<void> {
    try {
      const cleanData = Object.fromEntries(
        Object.entries(profileData).filter(([, v]) => v !== undefined)
      );
      ...
      if (res.status === 200) {
        ...
        console.error('Ошибка при обновлении пароля:', res.status, res.responseText);
      }
    } catch (error) {
  ...
  public async updateAvatar(file: File): Promise<void> {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        console.error('Ошибка при обновлении аватара:', res.status, res.responseText);
      }
    } catch (error) {
